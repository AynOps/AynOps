...
| [`dns_enumeration`](docs/tools/dns_enumeration.md) | `domain: str` | Multi-record DNS resolver (`dnspython`) + concurrent dictionary brute-forcing | Core Resource Records (`A`, `AAAA`, `MX`, `NS`, `TXT`, `CNAME`, `SOA`, `CAA`), enterprise SRV records, DNSSEC records (`DNSKEY`, `DS`, `RRSIG`, `NSEC`), CNAME chain tracing, reverse DNS PTR lookups, and active subdomains. | [Available](docs/tools/dns_enumeration.md) |
