...
def _format_signals_block(signals: dict) -> str:
    ...
    lines.append(
        f"Domain expiry      : {signals.get('domain_expiry_days', 'unknown')} days"
    )
    lines.append(
        f"SSL days remaining : {signals.get('ssl_days_remaining', 'unknown')} days"
    )
    lines.append(
        f"Open ports         : {', '.join(signals['open_ports']) or 'none detected'}"
    )
    lines.append(
        f"Software detected  : {', '.join(signals['software_detected']) or 'none'}"
    )
    ...
    lines.append(
        f"Missing sec headers: {len(missing_hdrs)} — {', '.join(missing_hdrs) or 'none'}"
    )
    ...
    if email_sec:
        lines.append(
            f"Email security score: {email_sec.get('security_score', 'n/a')} ({email_sec.get('rating', 'n/a')})"
        )
        lines.append(
            f"  SPF  : {'✓ found' if email_sec.get('spf_found') else '✗ missing'} — policy: {email_sec.get('spf_policy', 'n/a')}"
        )
        lines.append(
            f"  DKIM : {'✓ found' if email_sec.get('dkim_found') else '✗ missing'}"
        )
        lines.append(
            f"  DMARC: {'✓ found' if email_sec.get('dmarc_found') else '✗ missing'} — policy: {email_sec.get('dmarc_policy', 'n/a')}"
        )
    ...
    def execute_wave(wave_number):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(tools)) as executor:
            ...
            for tool in tools:
                ...
                if not tool.get("should_run", lambda domain, results: True)(
                    domain, results
                ):
                    results[tool["name"]] = {
                        "success": False,
