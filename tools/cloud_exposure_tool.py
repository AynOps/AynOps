...
def url_response(url: str) -> dict:
    ...
    try:
        ...
        return {"url": url, "status": "ERROR", "severity": "INFO", "note": str(e)}
...
def check_provider(bucket, provider):
    ...
    return {"bucket_name": bucket, "provider": provider, **result}
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                findings.append({"status": "ERROR", "note": str(e)})

    total_exposed = len([item for item in findings if item.get("status") == "PUBLIC"])
    total_private = len(
        [item for item in findings if item.get("status") == "EXISTS_PRIVATE"]
    )
    total_not_found = len(
        [item for item in findings if item.get("status") == "NOT_FOUND"]
    )
