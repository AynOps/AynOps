...
class TestSslInspect(unittest.TestCase):
    def _make_cert(
        self, days_valid=365, days_since_not_before=30, sans=None, extra=None
    ):
        now = datetime.now(UTC)
        not_before = (now - timedelta(days=days_since_not_before)).strftime(
            "%b %d %H:%M:%S %Y GMT"
        )
    ...
    def _run_inspect(
        ...
        if der_bytes is not None:
            mock_ssl_sock.getpeercert.side_effect = lambda binary_form=False: (
                der_bytes if binary_form else cert
            )
        else:
    ...
    def test_ssl_success(self, mock_ctx_cls, mock_create_connection):
        ...
        self.assertTrue(
            result["success"], f"Expected success but got error: {result.get('error')}"
        )
