...
class TestEmailSecurityScanner(unittest.TestCase):
    ...
    def test_parse_dkim_record(self):
        ...
        # Active key
        status, _ = _parse_dkim_record("v=DKIM1; k=rsa; p=MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8A")
        self.assertEqual(status, "active")
        # The v= tag is optional — a valid p= alone is still an active record
        self.assertEqual(_parse_dkim_record("p=MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8A")[0], "active")
        ...
        self.assertEqual(_parse_dkim_record("v=DKIM1; p=not-a-public-key!")[0], "malformed")
        # Unrelated TXT records are not DKIM records
        self.assertEqual(_parse_dkim_record("google-site-verification=abc123")[0], "not_dkim")
    ...
    def test_multiple_spf_records_invalid(self, mock_valid, mock_discover, mock_query):
        ...
        self.assertTrue(any("Multiple SPF records" in r for r in result["recommendations"]))
    ...
    @patch('tools.email_security_tool._discover_dynamic_selectors')
    @patch('tools.email_security_tool.is_valid_domain', return_value=True)
    def test_multiple_dmarc_records_invalid(self, mock_valid, mock_discover, mock_query):
        ...
        self.assertTrue(any("Multiple DMARC records" in r for r in result["recommendations"]))
    ...
    def test_dkim_unrelated_txt_ignored(self, mock_valid, mock_discover, mock_query):
        ...
        self.assertTrue(any("may still be configured" in r for r in result["recommendations"]))
    ...
    def test_spf_missing_all_mechanism(self, mock_valid, mock_discover, mock_query):
        ...
        self.assertTrue(any("no 'all' mechanism" in r for r in result["recommendations"]))
